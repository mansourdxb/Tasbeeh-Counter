...
import indexData from "@/data/the_9_books/bukhari/index.json";
...
export default function BukhariBooksScreen() {
  ...
  const books = (indexData as BukhariData).books ?? [];

  const filteredBooks = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return books;
    return books.filter((item) =>
      item.en.toLowerCase().includes(q) || item.ar.includes(query)
    );
  }, [books, query]);
  ...
  const title = (indexData as BukhariData).titleAr;
          ...
          <View style={styles.list}>
            {filteredBooks.map((item, index) => {
              ...
              return (
                <View key={item.id}>
                  <Pressable
                    onPress={() => navigation.navigate("BukhariChapter", { chapterId: item.id })}
                    style={[
                      styles.rowOuter,
