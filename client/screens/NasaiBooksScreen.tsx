...
import indexData from "@/data/the_9_books/nasai/index.json";
...
export default function NasaiBooksScreen() {
  ...
  const books = (indexData as NasaiData).books ?? [];
  ...
  const title = (indexData as NasaiData).titleAr;
            ...
            {filteredBooks.map((item, index) => {
              ...
              return (
                <View key={item.id}>
                  <Pressable
                    onPress={() =>
                      navigation.navigate("NasaiChapter", { chapterId: item.id })
                    }
                    style={[
                      styles.rowOuter,
