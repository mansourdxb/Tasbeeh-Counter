...
import indexData from "@/data/the_9_books/malik/index.json";
...
export default function MalikBooksScreen() {
  ...
  const books = (indexData as MalikData).books ?? [];
  ...
  const title = (indexData as MalikData).titleAr;
            ...
            {filteredBooks.map((item, index) => {
              ...
              return (
                <View key={item.id}>
                  <Pressable
                    onPress={() =>
                      navigation.navigate("MalikChapter", { chapterId: item.id })
                    }
                    style={[
                      styles.rowOuter,
