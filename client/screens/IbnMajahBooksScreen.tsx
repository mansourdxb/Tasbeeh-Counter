...
import indexData from "@/data/the_9_books/ibnmajah/index.json";
...
export default function IbnMajahBooksScreen() {
  ...
  const books = (indexData as IbnMajahData).books ?? [];
  ...
  const title = (indexData as IbnMajahData).titleAr;
            ...
            {filteredBooks.map((item, index) => {
              ...
              return (
                <View key={item.id}>
                  <Pressable
                    onPress={() =>
                      navigation.navigate("IbnMajahChapter", { chapterId: item.id })
                    }
                    style={[
                      styles.rowOuter,
