...
import indexData from "@/data/the_9_books/darimi/index.json";
...
export default function DarimiBooksScreen() {
  ...
  const books = (indexData as DarimiData).books ?? [];
  ...
  const title = (indexData as DarimiData).titleAr;
            ...
            {filteredBooks.map((item, index) => {
              ...
              return (
                <View key={item.id}>
                  <Pressable
                    onPress={() =>
                      navigation.navigate("DarimiChapter", { chapterId: item.id })
                    }
                    style={[
                      styles.rowOuter,
