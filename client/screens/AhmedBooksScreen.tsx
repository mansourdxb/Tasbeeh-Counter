...
import indexData from "@/data/the_9_books/ahmed/index.json";
...
export default function AhmedBooksScreen() {
  ...
  const books = (indexData as AhmedData).books ?? [];
  ...
  const title = (indexData as AhmedData).titleAr;
            ...
            {filteredBooks.map((item, index) => {
              ...
              return (
                <View key={item.id}>
                  <Pressable
                    onPress={() =>
                      navigation.navigate("AhmedChapter", { chapterId: item.id })
                    }
                    style={[
                      styles.rowOuter,
